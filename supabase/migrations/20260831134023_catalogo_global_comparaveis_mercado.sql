...
drop policy if exists "select_catalogo_comparaveis_mercado" on public.comparaveis_mercado;
create policy "select_catalogo_comparaveis_mercado" on public.comparaveis_mercado
  for select to authenticated using (true);

create index if not exists idx_comparaveis_mercado_catalogo_busca
  on public.comparaveis_mercado (cidade_chave, finalidade, ultimo_visto_em desc);
create index if not exists idx_comparaveis_mercado_catalogo_bairro_tipo
  on public.comparaveis_mercado (cidade_chave, bairro_chave, tipo);

create or replace function public.buscar_comparaveis_mercado_hibridos(
