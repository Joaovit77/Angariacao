...
describe("painel por estado atual e atenção (C9.1)", () => {
  it("hierarquia: cabeçalho humano, atenção só quando há, o que sabemos agora, ações recolhidas, visto anteriormente, passagens, localização, detalhes fechados", () => {
    ...
    // C13C: a memória do imóvel (o que a web disse) vem logo depois do que
    // sabemos em campo, antes das ações.
    expect(titulos).toEqual(["Precisa de atenção", "Próximas ações", "O que sabemos agora", "Memória do imóvel", "Ações", "Visto anteriormente", "Histórico de passagens", "Localização"]);
...
describe("refino de textos, data e tipografia (C9.1)", () => {
  ...
  it("desktop: lista estreita e presa ao rolar; painel em duas colunas pela largura do próprio painel, com decidir à esquerda e consultar à direita", () => {
    ...
    expect(titulos("principal")).toEqual(["Precisa de atenção", "Próximas ações", "O que sabemos agora", "Memória do imóvel", "Ações", "Visto anteriormente"]);
...
describe("informar o endereço depois do cadastro", () => {
  ...
  it("salvar só quando algo mudou e o endereço identifica o lugar; envia os dez campos e nada mais", () => {
    ...
    expect(salvar().disabled).toBe(true);
    fireEvent.change(screen.getByLabelText("Logradouro"), { target: { value: "Rua das Palmeiras" } });
    fireEvent.change(screen.getByLabelText("Número"), { target: { value: "120" } });
    fireEvent.change(screen.getByLabelText("Estado"), { target: { value: "pr" } });
    fireEvent.change(screen.getByLabelText("CEP"), { target: { value: "86010000" } });
    expect(salvar().disabled).toBe(false);
    fireEvent.click(salvar());
    expect(cenario.estado.definirEndereco).toHaveBeenCalledWith("identificado-1", {
      logradouro: "Rua das Palmeiras", numero: "120", unidade: "", bloco: "", edificio: "",
      bairro: "", cidade: "Londrina", estado: "PR", cep: "86010-000", pontoReferencia: "",
    });
