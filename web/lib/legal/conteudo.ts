  ...
  secoes: [
    ...
    },
    {
      ...
      paragrafos: [
        ...
        `**Do Garimpo em Campo:** o endereço do imóvel visto na rua, a data e o horário de cada passagem, a observação que você escrever (a interface orienta a não registrar nome nem telefone ali) e a foto da fachada, guardada em armazenamento privado, sem os metadados da câmera e acessível apenas pela sua conta.`,
        `**Localização do dispositivo:** somente no Garimpo em Campo, ao registrar uma passagem por um imóvel, o sistema pede ao navegador a posição atual do aparelho, uma única vez a cada registro e mediante a permissão que o próprio navegador solicita. Se você autorizar, são guardadas a latitude, a longitude e a precisão informada pelo aparelho, associadas àquela passagem, com a finalidade de localizar o imóvel; se recusar, a posição é estimada a partir do endereço digitado. Não há rastreamento contínuo nem coleta em segundo plano: a localização só é lida com a tela de registro aberta.`,
        `O sistema não usa cookies de publicidade e não faz rastreamento entre sites.`,
      ],
    },
    {
      ...
      paragrafos: [
        ...
        `- Registrar, por sua ação, os imóveis que você vê em campo, com foto, localização e observações, e sugerir, a partir do texto da observação, sinais como placa ou imóvel fechado — sempre como sugestão, sujeita à sua confirmação.`,
    ...
    },
    {
      ...
      paragrafos: [
        ...
        `- **OpenAI** — transcrição dos áudios recebidos, interpretação das respostas dos proprietários, geração dos textos sugeridos e, no Garimpo em Campo, a leitura das observações que você escreve sobre um imóvel para sugerir sinais de um catálogo fechado. Isso significa que o conteúdo das mensagens recebidas, incluindo áudios, e o texto dessas observações são enviados a esse fornecedor para processamento. Pelos termos de uso da API que utilizamos, esse conteúdo não é usado para treinar modelos.`,
