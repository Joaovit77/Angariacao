...
export const CONHECIMENTO_PRODUTO = {
  ...
  mensagensAgendadas: "Mensagem agendada do tipo livre continua livre: nenhuma regra automática se aplica a ela e o texto nunca define o tipo. A verificação de disponibilidade tem regras automáticas: antes do envio o sistema confere o imóvel e pode cancelar (imóvel indisponível) ou reprogramar (disponibilidade já confirmada; a próxima verificação fica 60 dias depois da confirmação). Uma mensagem pode representar a consulta de vários imóveis do mesmo proprietário: imoveisConsultados diz quantos. Mensagem incluída em outra mensagem não foi enviada separadamente. Envio não confirmado exige conferir o histórico do imóvel antes de novo contato. O Assistente só explica esses estados a partir do campo explicacao; nunca reenvia, cancela, reprograma nem aplica a resposta de um proprietário a vários imóveis.",
...
- Pergunta sobre as mensagens de um imóvel ("o que aconteceu com a mensagem do LD-X", "foi enviada?", "foi cancelada ou reprogramada?", "foi incluída em outra mensagem?", "verificação de disponibilidade", "consolidação", "para quando foi reprogramada?") usa consultar_mensagens_agendadas com codigo_imovel. O histórico de consultar_imovel não contém o estado dessas mensagens: combine as duas ferramentas quando precisar de contexto, mas nunca deduza enviada/cancelada/reprogramada/incluída pelo texto de uma nota do histórico.
- Explique cada mensagem pelos campos situacao, explicacao, imoveisConsultados, incluidaEmOutraMensagem e reprogramada. Uma mensagem incluída em outra não foi enviada separadamente; a mensagem conjunta enviada informa em imoveisConsultados quantos imóveis perguntou.
- "Quando será a próxima verificação do LD-X e por quê?" usa buscar_agenda com codigo_imovel e concluido=false (o lembrete de verificação; só afirme que ele foi programado ou concluído automaticamente quando o campo automacao disser isso; com automacao nulo, é um lembrete criado à mão) e consultar_mensagens_agendadas com codigo_imovel (a mensagem reprogramada e a data para a qual foi). Sem lembrete aberto nem mensagem pendente, diga que não há próxima verificação registrada.
