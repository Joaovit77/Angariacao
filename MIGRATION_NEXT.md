...
| A2 ✅ | **O drawer do Pipeline lia `imovel.fotos`**, campo que nenhum mapeador produz — sempre caía em "Sem fotos cadastradas.". | `web/components/pipeline/PipelineView.tsx` | **Resolvido (2026-07-10):** seção "Fotos" removida do drawer (bloco morto). Decisão do usuário. |
| A3 ✅ | **A "Conversão" do relatório usava definição própria** (locados ÷ angariados no período), diferente do Dashboard (locados ÷ processos fechados) — Julho mostrava 100% no relatório e 33% no Dashboard. | `web/lib/calculo/relatorios.ts` | **Resolvido (2026-07-10):** alinhada à fórmula do Dashboard (locados ÷ processos fechados), mantida escopada ao período. Julho passou a mostrar **50%**. Baseline e seu teste atualizados marcando a divergência intencional. |
| A4 ✅ | **`tempoAteLocacao` pode ser negativo**; comissão "recebida" em imóvel não-locado vale 0. | `web/lib/calculo/motor.ts` | **Investigado (2026-07-10): não são bugs de fórmula, e sim guardas contra dado inconsistente.** (1) `tempoAteLocacao` = `daysBetween(dataAngariacao, entrouEmLocado)`; só dá negativo se a data de locação for anterior à do primeiro contato — impossível num fluxo normal, e o filtro `t >= 0` em `metricsForRange` protege a média. (2) `comissaoRecebidaValor` só conta quando `status === "Locado"`; e o `salvarImovel` já zera `comissaoRecebida`/valor/data quando o status sai de Locado — então "recebida em não-locado" só surge com dado legado/editado à mão. **Nenhuma mudança de código.** Melhoria opcional futura: validar na entrada que `dataAngariacao` não seja posterior a uma data de status no histórico. |
