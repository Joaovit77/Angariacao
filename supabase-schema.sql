...
create index if not exists assistente_acoes_confirmado_por_idx
  on assistente_acoes (confirmed_by)
...
--
-- Configuração independente da Central e do Radar. Nesta fase os campos
-- operacionais ficam inertes: criar um mercado não agenda nem executa coleta.
-- ------------------------------------------------------------
create table if not exists mercados_monitorados (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
...
alter table mercados_monitorados enable row level security;

drop trigger if exists trg_mercados_monitorados_updated_at on mercados_monitorados;
create trigger trg_mercados_monitorados_updated_at
  before update on mercados_monitorados
  for each row execute function set_updated_at();

drop policy if exists "select_own_mercados_monitorados" on mercados_monitorados;
create policy "select_own_mercados_monitorados" on mercados_monitorados
  for select to authenticated using ((select auth.uid()) = user_id);

drop policy if exists "insert_own_mercados_monitorados" on mercados_monitorados;
create policy "insert_own_mercados_monitorados" on mercados_monitorados
  for insert to authenticated with check ((select auth.uid()) = user_id);

drop policy if exists "update_own_mercados_monitorados" on mercados_monitorados;
create policy "update_own_mercados_monitorados" on mercados_monitorados
  for update to authenticated
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);

drop policy if exists "delete_own_mercados_monitorados" on mercados_monitorados;
create policy "delete_own_mercados_monitorados" on mercados_monitorados
  for delete to authenticated using ((select auth.uid()) = user_id);

create index if not exists idx_mercados_monitorados_user_ativo
  on mercados_monitorados (user_id, ativo);
...
-- Canonicaliza somente UFs já explicitamente persistidas. NULL permanece
-- NULL: não há inferência de Paraná para linhas antigas sem evidência.
update comparaveis_mercado
...
create index if not exists idx_comparaveis_mercado_busca
  on comparaveis_mercado (user_id, estado, cidade_chave, finalidade, ultimo_visto_em desc);
create index if not exists idx_comparaveis_mercado_bairro_tipo
  on comparaveis_mercado (user_id, estado, cidade_chave, bairro_chave, tipo);
...
create index if not exists idx_comparaveis_mercado_catalogo_busca
  on comparaveis_mercado (estado, cidade_chave, finalidade, ultimo_visto_em desc);
create index if not exists idx_comparaveis_mercado_catalogo_bairro_tipo
  on comparaveis_mercado (estado, cidade_chave, bairro_chave, tipo);
...
-- Overload temporário de compatibilidade para deployments anteriores à Fase 5A.
-- A versão nova, logo abaixo, exige UF; a remoção desta assinatura antiga é
-- uma etapa de contract futura, após a estabilização do deploy em Production.
...
create table if not exists radar_buscas (
  ...
  ultimo_check_automatico timestamptz,
  ultimo_check_origem text check (ultimo_check_origem in ('manual', 'navegador', 'cron')),
  created_at timestamptz not null default now()
);

-- A service role registra o cron. O browser pode registrar somente as duas
-- origens interativas e nunca alterar o relógio reservado ao agendamento.
...
create index if not exists idx_radar_buscas_automatico_ativo
  on radar_buscas (ultimo_check_automatico, created_at) where ativo;
...
revoke all on table
  ...
  observacoes_comparaveis_mercado, mercados_monitorados
from public, anon, authenticated, service_role;
...
grant select, insert, update, delete on table mercados_monitorados to service_role;
...
create table if not exists public.politicas_repasse (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
...
create unique index if not exists politicas_repasse_padrao_ativa_uidx
  on public.politicas_repasse(user_id)
  where ativo and padrao;
create index if not exists politicas_repasse_user_idx
  on public.politicas_repasse(user_id, ativo, nome);

drop trigger if exists trg_politicas_repasse_updated_at on public.politicas_repasse;
...
create unique index if not exists locacoes_evento_origem_uidx
  on public.locacoes(user_id, evento_origem_chave)
  where evento_origem_chave is not null;
create index if not exists locacoes_imovel_idx
...
create index if not exists repasses_pendentes_idx
  on public.repasses(user_id, data_prevista, id)
  where status = 'pendente';
create index if not exists repasses_imovel_idx
  on public.repasses(user_id, imovel_id, created_at desc);
create index if not exists repasses_recebimento_operacao_idx
...
drop trigger if exists trg_locacoes_tenant on public.locacoes;
...
drop trigger if exists trg_repasses_tenant on public.repasses;
...
drop policy if exists "select_own_politicas_repasse" on public.politicas_repasse;
create policy "select_own_politicas_repasse" on public.politicas_repasse
  for select to authenticated
  using ((select auth.uid()) = user_id);
drop policy if exists "insert_own_politicas_repasse" on public.politicas_repasse;
create policy "insert_own_politicas_repasse" on public.politicas_repasse
  for insert to authenticated
  with check ((select auth.uid()) = user_id);
drop policy if exists "update_own_politicas_repasse" on public.politicas_repasse;
create policy "update_own_politicas_repasse" on public.politicas_repasse
  for update to authenticated
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);

drop policy if exists "select_own_locacoes" on public.locacoes;
create policy "select_own_locacoes" on public.locacoes
  for select to authenticated
  using ((select auth.uid()) = user_id);
drop policy if exists "select_own_repasses" on public.repasses;
...
drop trigger if exists trg_repasses_snapshot_immutavel on public.repasses;
...
-- Necessário para a CHECK de politicas_repasse rodar durante INSERT/UPDATE
-- autenticado. O schema private continua sem USAGE e a função não é exposta
-- como RPC pelo PostgREST.
...
-- ============================================================
-- CONTATOS — Fase 1a-A (espelho da migration
-- 20260921183930_contatos_fase1a.sql; o teste tests/contatos-schema.test.ts
-- exige que este bloco seja idêntico ao arquivo da migration).
-- ============================================================
-- ============================================================
